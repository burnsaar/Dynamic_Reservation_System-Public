...
def gen_vehicles_and_parameters_sensitivity(replications, numSpots, demands, truckProps, nhts_data,
            ...
            for demand in demands:
                ...
                for numVehicles in totalNumVehicles:
                    for truckProp in truckProps:
                        ...
                        tempArg = ('FCFS', 'Baseline FCFS', numSpot, baseData, 0, 0, 5, 1, 0, 'N/A', i, j, 60, 0, 0) #run FCFS, scenario FCFS
                        args.append(tempArg)
                        i += 1
                        ...
                        for receivedDelta in receivedDeltas:
                            ...
                            if receivedDelta != -1: #we are at scenario #2
                                ...
                                tempArg = ('full', 'Baseline FD', numSpot, tempData, 0, 0, 5, 1, 0, receivedDelta, i, j, 60, 0, 0) #append scenario #FD
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                tempArg = ('SW','scenario 1', numSpot, tempData, 0, 0, 5, 1, 0, receivedDelta, i, j, 60, 0, 0) #append scenario #1
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                tempArg = ('SW','scenario 1a', numSpot, tempData, 0, 0, 5, 1, 0, receivedDelta, i, j, 30, 0, 0) #append scenario #1a, change tau to 30
                                args.append(tempArg)
                                i += 1
                                print(i) 
                            else:
                                ...
                                tempArg = ('SW', 'scenario 2', numSpot, tempData, 0, 0, 5, 1, 0, receivedDelta, i, j, 60, 0, 0) #append scenario #2
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                tempArg = ('SW', 'scenario 2a', numSpot, tempData_shift10, 10, 0, 5, 1, 0, receivedDelta, i, j, 60, 0, 0) #append scenario #2a,tempData60
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                tempArg = ('SW', 'scenario 2b', numSpot, tempData15, 0, 0, 5, 1, 0, receivedDelta, i, j, 60, 0, 0) #append scenario #2b, tempData15
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                tempArg = ('SW', 'scenario 2c', numSpot, tempData5, 0, 0, 5, 1, 0, receivedDelta, i, j, 10, 0, 0) #append scenario #2c, tempData5, tau=10
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                
                                tempArg = ('SW', 'scenario 3', numSpot, tempData, 0, 0, 5, 1, 0, receivedDelta, i, j, 60, 15, 15) #append scenario #3
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                tempArg = ('SW', 'scenario 3a', numSpot, tempData_shift10, 10, 0, 5, 1, 0, receivedDelta, i, j, 60, 15, 15) #append scenario #3
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                tempArg = ('SW', 'scenario 3b', numSpot, tempData15, 0, 0, 5, 1, 0, receivedDelta, i, j, 60, 15, 15) #append scenario #3
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                tempArg = ('SW', 'scenario 3c', numSpot, tempData5, 0, 0, 5, 1, 0, receivedDelta, i, j, 10, 5, 5) #append scenario #3
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                tempArg = ('SW', 'scenario 3d', numSpot, tempData_shift10, 10, 0, 5, 1, 0, receivedDelta, i, j, 60, 15, 0) #append scenario #3
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                
                                r_i = np.subtract(tempData.loc[:,  'a_i_OG'], tempData.loc[:, 'Received'])
                                tempArg = ('SW', 'scenario 4', numSpot, tempData, 0, 0, 5, 1, 0, receivedDelta, i, j, 60, r_i, r_i) #append scenario #5
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                r_i = np.subtract(tempData_shift10.loc[:,  'a_i_OG'], tempData_shift10.loc[:, 'Received'])
                                tempArg = ('SW', 'scenario 4a', numSpot, tempData_shift10, 10, 0, 5, 1, 0, receivedDelta, i, j, 60, r_i, r_i) #append scenario #5
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                #do not need a scenarion 4b or 4c b/c r_i, pho, nu = 15 is covered by scenario 3b and 3c
                                r_i = np.subtract(tempData_shift10.loc[:,  'a_i_OG'], tempData_shift10.loc[:, 'Received'])
                                tempArg = ('SW', 'scenario 4d', numSpot, tempData_shift10, 10, 0, 5, 1, 0, receivedDelta, i, j, 60, r_i, 0) #append scenario #5
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                
                                r_i = np.subtract(tempData.loc[:,  'a_i_OG'], tempData.loc[:, 'Received'])
                                tempArg = ('SW', 'scenario 5', numSpot, tempData, 0, 5, 5, 1, 0, receivedDelta, i, j, 60, r_i, r_i) #append scenario #6
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                r_i = np.subtract(tempData_shift10.loc[:,  'a_i_OG'], tempData_shift10.loc[:, 'Received'])
                                tempArg = ('SW', 'scenario 5a', numSpot, tempData_shift10, 10, 5, 5, 1, 0, receivedDelta, i, j, 60, r_i, r_i) #append scenario #6
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                r_i = np.subtract(tempData15.loc[:,  'a_i_OG'], tempData15.loc[:, 'Received'])
                                tempArg = ('SW', 'scenario 5b', numSpot, tempData15, 0, 5, 5, 1, 0, receivedDelta, i, j, 60, r_i, r_i) #append scenario #6
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                r_i = np.subtract(tempData5.loc[:,  'a_i_OG'], tempData5.loc[:, 'Received'])
                                tempArg = ('SW', 'scenario 5c', numSpot, tempData5, 0, 5, 5, 1, 0, receivedDelta, i, j, 10, r_i, r_i) #append scenario #6
                                args.append(tempArg)
                                i += 1
                                print(i) 
                                r_i = np.subtract(tempData.loc[:,  'a_i_OG'], tempData.loc[:, 'Received'])
                                tempArg = ('SW', 'scenario 5e', numSpot, tempData, 0, 15, 5, 1, 0, receivedDelta, i, j, 60, r_i, r_i) #append scenario #6
                                args.append(tempArg)
                                i += 1
                                print(i)
    ...
    #saveFile = '/ocean/projects/eng240001p/aburns3/' + str(current_date) + '_Args_full_ILP_test.dat'
    ...
    #saveFile_length = '/ocean/projects/eng240001p/aburns3/' + str(current_date) + '_Args_length_full_ILP_test.txt'
...
if __name__ == '__main__':
    ...
    current_date = current_date + '_Aaron Result_full_ILP_test'
    ...
    #     runFullSetOfResults(arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6], arg[7], arg[8], arg[9], arg[10], arg[11], arg[12], arg[13], arg[14])
    #     scenario +=1
