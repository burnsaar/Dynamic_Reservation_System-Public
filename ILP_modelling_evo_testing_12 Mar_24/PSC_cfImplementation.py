import pandas as p
import numpy as np
from PSC_SimulateNHTSData import load_nhts_data, select_n_trips, construct_truth_dataframe, gen_truck_arrivals, join_requests
...
def runFullSetOfResults(algo, scenario, numSpots, data, phi, buffer, zeta, weightDoubleParking, weightCruising, received_delta, saveIndex=0, dataIndex=0, tau=0, rho=0, nu=0):
    ...
    if algo == 'FCFS':
        ...
        try:
            ...
            print('bids = ')
            print(bids)
            runtime, status, obj, dbl_park_Opt, park_demand, end_state_x_i_j, dbl_park_events, park_events = AP.AP(len(fullData), end=2400, Q=fullData, num_spaces=numSpots, bids=bids, flex=0, buffer=buffer, x_initialize=[None], timelimit=100)
