...
def AP(num_trucks, end, Q, num_spaces, bids, flex, buffer, x_initialize, timelimit = 120):
    ...
    #adjust Q to align with the supercomputer and paper 2 requirements
    Q['a_i'] = Q['a_i_OG']
    Q['d_i'] = Q['a_i_OG'] + Q['s_i']
    Q.reset_index(inplace=True)
    
    
    #start the optimzation framework
    t0 = datetime.now()
    x = [(i, j) 
    ...
    for i in range(0, len(Q)):
        ...
        if x_i_j.sum(i+1, '*').getValue() == 0:
            ...
            dbl_parked_data.append(Q["Vehicle"][i])
