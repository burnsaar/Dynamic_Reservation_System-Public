...
while current_time < end_scenario:
    ...
    Q['Prev Assigned'] = pd.concat(
        [req_incoming_tau['Assigned'], req_old_no_tau['Assigned'], req_old_yes_tau['Assigned'],
         req_old_yes_future_tau['Assigned']])
    ...
    if Q.empty == False:  # e.g. there are vehicle requests to consider in this time window
        status, obj, count_b_i, end_state_t_i, end_state_x_ij, dbl_park_events, park_events = MOD_flex.MOD_flex(n_tau,
                                                                                                                c, Q,
                                                                                                                buffer,
                                                                                                                current_time,
                                                                                                                current_time + tau,
        ...
        status, obj, count_b_i, end_state_t_i, end_state_x_ij, dbl_park_events, park_events, total_min_dbl_parked_soln \
            = PAP_min_deviation.MOD_flex(n_tau, c, Q, buffer, current_time, current_time + tau, end_scenario,
        ...
        plot_data[current_time] = OrderedDict()
        for tempType in ['Yes', 'No', 'Temp']:
            plot_data[current_time][tempType] = np.sum(req_master.loc[req_master.loc[:, 'Assigned'] == tempType, 's_i'])
toc = time.time()
...
plot_data.loc[:, 'Time'] = plot_data.index

long_plot_data = []
for tempType in ['Yes', 'No', 'Temp']:
    tempData = plot_data.loc[:, ['Time', tempType]]
    tempData = tempData.rename({tempType: 'Parking Length'}, axis=1)
    tempData.loc[:, 'Parking Assignment'] = tempType
    long_plot_data.append(tempData)

long_plot_data = pd.concat(long_plot_data)
x = 4
gr = (1+np.sqrt(5))/2
y = x/gr
...
ax = sns.barplot(x='Time', y='Parking Length', hue='Parking Assignment', data=long_plot_data)
sns.move_legend(ax, 'upper left', bbox_to_anchor=(1,1))
plt.title('Parking Assignments Over Time')
plt.ylim([0,50])
plt.savefig('Plots/OldParkingAssignment-{}.png'.format(int(new_scenario)), bbox_inches='tight')
