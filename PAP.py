...
#                 for jointData in range(0, len(vertices)) if jointData != i]
#
#
...
#             for jointData in range(0, len(Q)):
#                   M_ij = min(end_scenario, Q['b_i'].iloc[i] + Q['phi'].iloc[i] + Q['s_i'].iloc[i] + buffer) - max(start, Q['a_i'].iloc[jointData] - Q['phi'].iloc[jointData])
#                   M_inst.append(M_ij)
#
#             M_df[i] = M_inst
#             M_inst = []
...
#
#         m.addConstrs( (t_i[jointData]/scale) >= ((t_i[i] + Q['s_i'].iloc[i] + buffer - ((1 - x_i_j[i+1, jointData+1])*M_df.iloc[i, jointData]))/scale) #go back to *M if going with the generic original M and not M_ij, M_df.iloc[i, jointData]
#                       for i in range(0, len(Q))
#                       for jointData in range(0, len(Q)) if jointData != i)
#
#
...
#                 t_0i = Q['a_i'].iloc[i]
#
#             m.addConstr( (t_i[i]/scale) >= (Q['a_i'].iloc[i] - (Q['phi'].iloc[i]*0) - ((1-x_i_j.sum(i+1, '*'))*(Q['a_i'].iloc[i] - (Q['phi'].iloc[i]*0) - t_0i)) )/scale)
...
def MOD_flex_cf(num_trucks, num_spaces, Q, buffer, start, end, end_scenario, t_initialize, x_initialize, objective_dict=None):
    # try:
        # added variables to help scale the constraints in the optimization algorithm
        import numpy as np
        newSMakeUpDict = dict(zip(['Double Park', 'Cruising'], ['s_i', 'Potential Cruising Time']))
        # Q.loc[:, 'old_s_i'] = Q.loc[:, 's_i']
        Q.loc[:, 'p_i'] = 0
        Q.loc[:, 'n_i'] = 0
        parked_obj = objective_dict['parked']
        weight_obj = objective_dict['weights']
        cols_obj = objective_dict['cols']

        if(len(parked_obj)!=len(weight_obj) or len(weight_obj)!=len(cols_obj)):
            print("Objective function isn't be specified correctly")

        for i in range(len(parked_obj)):
            temp_park_obj = parked_obj[i]
            temp_weight_obj = weight_obj[i]
            temp_cols_obj = cols_obj[i]
            print((temp_park_obj, temp_weight_obj, temp_cols_obj))
            if(temp_park_obj==1):
                obj_col = 'p_i'
            else:
                obj_col = 'n_i'
            print(Q.loc[:, temp_cols_obj].prod(axis=1))
            Q.loc[:, obj_col]=Q.loc[:, obj_col]+(Q.loc[:, temp_cols_obj].prod(axis=1)*temp_weight_obj)
        ...
        # New set of constraints that enforce previously made parking decisions;
        #Note, changes in specific parking times are kept based on the Q matrix that is passed
        #To the optimizer
        if(type(x_initialize)!=type(None)):
            print('X initialization:')
            print(x_initialize)
            print(type(x_initialize))
            print(len(x_initialize))
            print(x_i_j)
            print(arcs)
            print(len(arcs))
            temp_i = 0
            for i in range(x_initialize[1]):
                for j in range(x_initialize[1]):
                    if(i!=j):
                        x_i_j[i,j].start = x_initialize[0][temp_i]
                        x_i_j[i, j].lb = x_initialize[0][temp_i]
                        x_i_j[i, j].ub = x_initialize[0][temp_i]
                        temp_i+=1
            # for i in x_i_j:
            #     if(temp_i<len(x_initialize)):
            #         # # m.addConstr(x_i_j[i], GRB.EQUAL, x_initialize[temp_i], "lol{}".format(temp_i))
            #         # m.addConstr(x_i_j[i]==x_initialize[temp_i])
            #         # # print(i)
            #         # # print(x_initialize[temp_i])
            #         # x_i_j[i].start = x_initialize[temp_i]
            #         # x_i_j[i].lb = x_initialize[temp_i]
            #         # x_i_j[i].ub = x_initialize[temp_i]
            #         # # temp_i+=1
            #         pass
        m.update()
        # Flow balance (3)
        ...
        for i in t_i:
            ...
            m.addConstr((t_i[i] / scale) >= (Q['a_i'].iloc[i] - (Q['phi'].iloc[i]*0) - (
                        (1 - x_i_j.sum(i + 1, '*')) * (Q['a_i'].iloc[i] - (Q['phi'].iloc[i]*0) - t_0i))) / scale)
        ...
        obj = gp.LinExpr()
        for i in range(0, len(Q)):
            obj += (1 - x_i_j.sum(i + 1, '*')) * (Q['n_i'].iloc[i] / scale_n_i)  # double parking objective function
            obj += (- x_i_j.sum(i + 1, '*')) * (Q['p_i'].iloc[i] / scale_p_i)  # double parking objective function
            # obj += b_i[i] #cruising objective function
        # obj = gp.LinExpr()
        # print(weights)
        # print(Q.columns)
        # for i in range(0, len(Q)):
        #     obj += (-x_i_j.sum(i + 1, '*')) * (
        #             Q['s_i'].iloc[i] / scale_s_i * weights['parked'])
            # +(1 - x_i_j.sum(i + 1, '*')) * (
            #                     Q['s_i'].iloc[i] / scale_s_i * weights['double'] * Q['Double Park'])+(1 - x_i_j.sum(i + 1, '*')) * (
            #                     Q['s_i'].iloc[i] / scale_s_i * weights['cruise'] * Q['Cruising'].iloc[i] * Q['Potential Cruising Time']))
            # for obj_name, weight in weights.items():
            #     if(obj_name=='parked'):
            #         obj += (-x_i_j.sum(i + 1, '*')) * (
            #                     Q['s_i'].iloc[i] / scale_s_i * weight)  #parking objective function; negated
            #     elif(obj_name=='double'):
            #         obj += (1 - x_i_j.sum(i + 1, '*')) * (
            #                     Q['s_i'].iloc[i] / scale_s_i * weight * Q['Double Park'])  # double parking objective function
            #     elif(obj_name=='cruise'):
            #         obj += (1 - x_i_j.sum(i + 1, '*')) * (
            #                     Q['s_i'].iloc[i] / scale_s_i * weight * Q['Cruising'].iloc[i] * Q['Potential Cruising Time'])  # cruising objective function
            # obj += b_i[i] #cruising objective function
        ...
        # -----------------------------------------------------------------------------
        # Compute optimal solution
        # print('Start: {}'.format(m.getAttr("X")))
        m.optimize()
        # for tup, var in x_i_j.items():
        #     print('End: {}'.format(var.X))
        # print('\n')
        m.printQuality()
        ...
        for i in x_i_j:
            end_state_x_ij.append(int(x_i_j[i].getAttr("x")))
        print('lol-{}: {}'.format(start, m.getObjective().getValue()))
        return m.status, m.getObjective().getValue(), count_b_i, end_state_t_i, end_state_x_ij, dbl_park_events, park_events
